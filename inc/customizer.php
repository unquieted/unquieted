 ...
 */
function unquieted_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color', 
				array(
					'label'		=> __( 'Header and footer background color', 'unquieted'),
					'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'unquieted' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options', 
		array(
			'title'			=> __( 'Theme Options', 'unquieted' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'unquieted' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'unquieted_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'unquieted_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'unquieted' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'unquieted' ),
				'full-content'	=> __( 'Full content', 'unquieted' )
			),
...
add_action( 'customize_register', 'unquieted_customize_register' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function unquieted_customize_preview_js() {
	wp_enqueue_script( 'unquieted_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'unquieted_customize_preview_js' );
...
function unquieted_sanitize_length( $value ) {
...
if ( ! function_exists( 'unquieted_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see unquieted_custom_header_setup().
 */
function unquieted_header_style() {
